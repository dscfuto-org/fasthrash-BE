...
const WASTE_LABELS = ['plastic', 'water', 'bottle'];
const THRESHOLD = 0.7;
...
exports.visionAIFilter = async (req, res, next) => {
  ...
  try {
    for (let photo of photos) {
      ...
      let validLabels = labels.filter(
        ({ label, score }) =>
          WASTE_LABELS.includes(label.toLowerCase()) && score > THRESHOLD
      );
      ...
      if (validLabels.length > 0) continue;
        ...
        throw new Error(
          `${photo.originalname} was rejected. Please verify that your plastic waste is acceptable for recycling.`
        );
    }
